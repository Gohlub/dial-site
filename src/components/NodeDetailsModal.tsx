...
export const NodeDetailsModal: React.FC<{ node: UserNode }> = ({ node }) => {
    ...
    const regenerateSshPassword = async () => {
        if (
            !window.confirm(
                `Are you sure you want to regenerate the SSH password for ${node.kinode_name}.os?`,
            )
        ) {
            return
        }
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col place-items-center place-content-center self-center grow self-stretch gap-2">
                <span className="">
                    Created:{' '}
                    {dayjs(node.created_at).format('D MMM YYYY HH:mm:ss')} (
                    {dayjs(node.created_at).fromNow()})
                </span>
                <span>
                    Product: {node.product_title} - {node.product_description}
                </span>
                <div className="flex">
                    <span>Web Login:</span>
                    <a
                        ...
                        target="_blank"
                        className={classNames(
                            'flex items-center ml-2 font-bold',
                            {
                                'pointer-events-none':
                ...
                {showAdvanced && node.ssh_password && (
                    <div className="flex flex-col gap-2">
                        <div className="flex c">
                            <span>SSH Address:</span>
                            <span className="font-mono text-xs">
                                {node.ssh_username}@{node.ssh_address}
                            </span>
                            <button
                                className="icon"
                                onClick={() =>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                className="icon"
                                onClick={() =>
                                    copyToClipboard(node.ssh_password)
                                }
                            >
                                <FaCopy />
                            </button>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-mono text-xs">
                                {node.local_http_port}
                            </span>
                            <button
                                className="icon"
                                onClick={() =>
                                    copyToClipboard(node.local_http_port)
                                }
                            >
                                <FaCopy />
                            </button>
