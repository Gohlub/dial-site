...
export const ResetPasswordModal = () => {
    const { activeNode, setResetPasswordModalOpen, resetNodePassword } =
        useDialSiteStore()
    const [newPassword, setNewPassword] = useState<string>('')
    const [newPasswordHash, setNewPasswordHash] = useState<string>('')
    const [confirmNewPasswordHash, setConfirmNewPasswordHash] =
        useState<string>('')
    ...
    return (
        <Modal
            onClose={() => {
                passwordIsResetting ? void 0 : setResetPasswordModalOpen(false)
            }}
            title="Reset Password"
        >
            {passwordIsResetting ? (
                <div>
                    Resetting password. Please wait while the Ethereum operation
                    completes...
                </div>
            ) : (
                <>
                    ...
                    >
                        <input
                            ...
                            className={classNames('grow', {
                                'w-full mb-2': isMobile,
                                'mr-2': !isMobile,
                            })}
                            onChange={(e) =>
                                onNewPasswordChanged(e.target.value)
                            }
                            minLength={8}
                        />
                        <input
