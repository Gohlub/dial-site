...
export const Alerts: React.FC = () => {
  ...
  return <div className={classNames('fixed flex flex-col place-items-center max-h-48 overflow-y-auto', {
    ...
    {alerts
      .filter((a, i, alerts) => !a.dismissed && alerts.findIndex(b => b.id === a.id) === i)
      .slice()
      .reverse()
      .map((alert, i) => <div
        key={alert.id}
        className={classNames('flex gap-2 text-white py-2 px-4 rounded-md mb-2 backdrop-blur-md relative', {
          'bg-red-500/25': alert.class === 'danger' || alert.class === 'alert',
          'bg-yellow-500/25 text-black': alert.class === 'warning',
        ...
        <div className={classNames("flex flex-col gap-2", {
          ...
          <span className="mr-1">
            {dayjs(alert.start_time * 1000).format('MMMM DD, YYYY h:mm A')}:
          </span>
          <div>
            {alert.content}
          </div>
        </div>
        <div className={classNames("flex flex-col gap-2 self-stretch", {
          'w-1/8': !isMobile
        })}>
          <button
