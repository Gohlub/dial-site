...
export const AddNodeModal = () => {
  ...
  const onCheckAvailable = async () => {
    ...
    if (nodeName.length < 9) return alert('Node name must be at least 9 characters long.')
    if (!nodeName.match(/^[a-z0-9-]+$/)) return alert('Node name must be alphanumeric, with dashes allowed.')
  ...
  return <Modal title="Add a new node" onClose={() => setAddNodeModalOpen(false)}>
    <span className='text-sm'>{kinodesRemaining} / {userInfo?.maxKinodes} nodes remaining</span>
    {kinodesRemaining <= 0
      ? <>
        <div className='flex flex-col self-stretch text-center place-items-center place-content-center gap-2'>
          <span>Your account has claimed its free node{userNodes.length > 1 ? 's' : ''}.</span>
          <span>To manage additional nodes, please contact Kinode support: <a href="mailto:admin@kinode.com">admin@kinode.com</a>.</span>
        </div>
      </>
      : <>
        <div className={classNames('flex place-items-center place-content-center', { 'flex-wrap': isMobile })}>
